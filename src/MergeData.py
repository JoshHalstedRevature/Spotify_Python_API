...
def ShapeDF(CSV_File, drop_column=None, reformat_column=None):
    ...
    new_df_columns = RemoveElementFromList(
        list(modified_df.columns), reformat_column)
    new_df = ExpandDF(modified_df, id_vars=new_df_columns,
                      rename_column=reformat_column)
    return new_df
...
def MergeCSVs():
    ...
    if Total_Data.is_file():
        ...
        Tracks_Df = ShapeDF(TRACKS_CSV, drop_column='artists',
                            reformat_column='id_artists')
        Tracks_Df.to_csv(os.path.join(CSV_DIRECTORY, "Track_Data.csv"))
        Renamed_Tracks_DF = RenameDFColumns(Tracks_Df, "id", "id_song")
        Renamed_Tracks_DF = RenameDFColumns(
            Renamed_Tracks_DF, "popularity", "song_popularity")
        Renamed_Tracks_DF = RenameDFColumns(
            Renamed_Tracks_DF, "name", "song_name")
        Artists_Df = ShapeDF(ARTISTS_CSV, reformat_column='genres')
        Renamed_Artists_DF = RenameDFColumns(Artists_Df, "id", "id_artists")
        Renamed_Artists_DF = RenameDFColumns(
            Renamed_Artists_DF, "popularity", "artist_popularity")
        Renamed_Artists_DF = RenameDFColumns(
            Renamed_Artists_DF, "name", "artist_name")
        Artists_Df.to_csv(os.path.join(CSV_DIRECTORY, "Artists_Data.csv"))
        Merged_DF = JoinDFs(Renamed_Tracks_DF, Renamed_Artists_DF,
                            left_key='id_artists', right_key='id_artists')
